...
const PPM1Router = express.Router();



PPM1Router.route('/:echelle/:limit')
.get((req,res,next) => {
    ...
    switch(req.params.echelle){
        case "perHour" :
            Parameter.aggregate([
                ...
                avg : {$avg:"$PPM1"},
                sum : {$sum:"$PPM1"}, //equivalent Pluie_cumulee par Heure
                min: {$min:"$PPM1"},
                max : {$max:"$PPM1"}},
        ...
        case "perDay" :
            Parameter.aggregate([{ $group : { 
                _id : { year: { $year : "$timestamp" },
                        ...
                        day: { $dayOfMonth : "$timestamp" }}, 
                avg : {$avg:"$PPM1"},
                sum : {$sum:"$PPM1"}, //equivalent Pluie_cumulee par parseInt(req.params.echelle) heure
                min: {$min:"$PPM1"},
                max : {$max:"$PPM1"}},
        ...
        case "instantanee":
            ...
            Parameter.find({},{PPM1:1 ,timestamp:1}).sort(sort).limit(limit).then(
